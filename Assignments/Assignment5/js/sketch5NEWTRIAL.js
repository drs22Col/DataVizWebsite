...
//Label Y axis numbers
  ...
  text(number[i], 20, map(number[i], 0, 55000, bottomY, topY));
  print(number[i]);
  textSize(18);
     // stroke(200);
...
//map functions: define variables for rectangles and get data 
    ...
    var q3map = map(q3, 0, 55000, rightX, 0);
 //
    var q1 = neighborhoodTable.getNum(i, 'Q1');
    var q1map = map(q1, 0, 55000, rightX, 0);

 //define median and map
    var median = neighborhoodTable.getNum(i, 'Median');
    var medianmap = map(median, 0, 60000, rightX, 0);

//define topbar var and map
    var topbar = neighborhoodTable.getNum(i, 'TOPBAR');
    var topbarmap = map(topbar, 0, 55000, rightX,0);

  //set color for rects
    fill(200, i*50, 80);

  //draw rectangles
    rect(i*130+50, q3map, 100, (q1map-q3map));
