...
function drawXandYaxes() { 
 ...
 function LabelXaxis(){ 
  for (var i = 0; i < 25; i++){
    text(zipcode[i], i*130+80, 1250);
    fill(150);

   // OR use this SO THAT THE PLACEMENT IS WORKED OUT THROUGH MAPPING??
...
function draw(){
 ...
 //
    var AvgValue = AvgAssessedTable.getNum(i, 'Average Assessed');
    var avgvalueMap = map(AvgValue, 0, 1600000, 600, 0);   //range of original to range of target

    //draw rectangles
     //set color for rects
    fill(200, i*50, 80);
    //rect(i*130+100, q3map, 100, (q1map-q3map));
    rect(i*130+100, 100, 100, avgvalueMap);   // (x,y,w, h) NOT SURE OF SECOND VALUE
  ...
  //draw rectangles
   ...
   //Draw vertical grid lines on the chart, separating the bars and zip code labels on x axis below
...
//   var colorFor10003 =colors[modeFor10003-1];
//   var modeFor10003 =getModeByZipcode(AllAssessedTable 10003);
//   var colorFor10001= colors[modeFor10001-1];
//     var modeFor10003 =getModeByZipcode(AllAssessedTable, 10003);
//     var colorFor10001= colors[modeFor10001-1];
//       var modeFor10003 =getModeByZipcode(AllAssessedTable, 10003);
//       var colorFor10001= colors[modeFor10001-1];
...
//   var zips = data.getRow("ZipCode"),   PUT IN OTHER DATA TABLE HERE
//   byZip = [];
...
//   }
// }
    //?Draw a few stars in sky to create more of a night sky effect 
//   CAN USE CODE BELOW FROM p5 REFERENCE (STARS DONT NEED TO ROTATE THOUGH)
//   push();
...
//FOR INTERACTIVE VERSION OF THE ABOVE CHART, MAKE A NEW SKETCH THAT KEEPS ALL OF THE ABOVE AND ADDS FOLLOWING:

   //function antennae(): Make the vertical lines for antennae of "buildings"/rectangles, in the center of each rectangle/bar
      //stroke(200, i*50, 80);
      //line(i*65+100,  , i*132+140, )

   //function drawlights(): to draw a small red circle (the light at the top of each antennae) at the 
...
//the address is in the column called Address of Maximum, and the value of that address is in MaxValue column
     ...
     // var MaxAddress = AvgAssessedTable, getNum(i, 'Address of Maximum')  BUT NOTE THERE ARE LETTERS NOT JUST NUMBERS IN ADDRESSES
   

   //*if i have the time i can take photos of the address of max for each zip from google earth and the photos could come up when rolloever  
//would save as 10001MaxPhoto, 10003MaxPhoto, etc. 


}
 
// ?SONG that plays when click on image of homeless person
//if (song) song.stop();
//   song = null;
//   if mousePress  
//           song = chocolateSong;
